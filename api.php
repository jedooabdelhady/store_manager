...
// =====================================================
// تحسينات الأمان - الخطوة 1: إنشاء CSRF Token
// =====================================================
...
// =====================================================
// معالجة البيانات عند الإرسال النهائي
// =====================================================
...
        echo json_encode(['status' => 'error', 'message' => 'خطأ في التحقق الأمني. يرجى إعادة المحاولة.']);
        exit;
    }
    
...
        echo json_encode(['status' => 'error', 'message' => 'كلمة المرور لا تستوفي الشروط (10 خانات، حرف كبير، ورقم).']);
        exit;
    }
    
...
    }
    
    // 4. التحقق من عدم تكرار البريد
    $stmt = $conn->prepare("SELECT id FROM merchants WHERE email = ? LIMIT 1");
    if (!$stmt) {
...
    
    if ($stmt->num_rows > 0) {
        echo json_encode(['status' => 'error', 'message' => 'البريد الإلكتروني مسجل بالفعل.']);
        $stmt->close();
        exit;
    }
    $stmt->close();
    
    // 5. التحقق من رقم الجوال (صيغة سعودية)
    $phone = isset($_POST['phone']) ? trim($_POST['phone']) : '';
    if (!preg_match('/^(05)[0-9]{8}$/', $phone)) {
        echo json_encode(['status' => 'error', 'message' => 'رقم الجوال يجب أن يبدأ بـ 05 و يتكون من 10 أرقام.']);
        exit;
    }
    
    // 6. دالة آمنة لرفع الملفات
    function uploadFile($fileKey, $allowedExt = ['jpg', 'jpeg', 'png', 'pdf']) {
...
        }
        
        // الحصول على امتداد الملف بشكل آمن
...
    if (!$doc_img || !$iban_img) {
        echo json_encode(['status' => 'error', 'message' => 'خطأ في رفع الملفات. تأكد من نوع وحجم الملفات.']);
        exit;
    }
    
    // 8. تجهيز البيانات بشكل آمن (Prepared Statements)
    $first_name = isset($_POST['first_name']) ? trim($_POST['first_name']) : '';
    $last_name = isset($_POST['last_name']) ? trim($_POST['last_name']) : '';
    $store_name = isset($_POST['store_name']) ? trim($_POST['store_name']) : '';
    $store_link = isset($_POST['store_link']) ? trim($_POST['store_link']) : '';
    $doc_type = isset($_POST['doc_type']) ? $_POST['doc_type'] : 'commercial';
    $doc_number = isset($_POST['doc_number']) ? trim($_POST['doc_number']) : '';
    $doc_expiry = isset($_POST['doc_expiry']) ? $_POST['doc_expiry'] : '';
    $national_address = isset($_POST['national_address']) ? trim($_POST['national_address']) : '';
    
    $is_vat = isset($_POST['is_vat']) ? 1 : 0;
    $vat_number = ($is_vat && isset($_POST['vat_number'])) ? trim($_POST['vat_number']) : null;
    
    $bank_name = isset($_POST['bank_name']) ? trim($_POST['bank_name']) : '';
    $iban = isset($_POST['iban']) ? strtoupper(trim($_POST['iban'])) : '';
    $swift_code = isset($_POST['swift_code']) ? strtoupper(trim($_POST['swift_code'])) : '';
...
    
    // 9. إدراج البيانات باستخدام Prepared Statements
    $insert_sql = "INSERT INTO merchants (
...
        status, phone_verified, email_verified, created_at
    ) VALUES (
        ?, ?, ?, ?, ?, ?,
        ?, ?, ?, ?, ?, ?, ?,
        ?, ?, ?,
...
        $bank_name, $iban, $swift_code, $beneficiary_name, $iban_img
    );
    
    // تنفيذ الاستعلام
    if ($stmt->execute()) {
...
        $log_stmt = $conn->prepare("INSERT INTO audit_logs (user_type, action, table_name, record_id, ip_address, user_agent) VALUES (?, ?, ?, ?, ?, ?)");
        
        if ($log_stmt) {
            $action = 'merchant_registration';
            $table = 'merchants';
            $log_stmt->bind_param("ssisis", $user_type, $action, $table, $merchant_id, $ip, $ua);
            $user_type = 'merchant';
            $log_stmt->execute();
            $log_stmt->close();
        }
...
    } else {
        echo json_encode(['status' => 'error', 'message' => 'خطأ في إدراج البيانات: ' . $stmt->error]);
        $stmt->close();
    }
    
    exit;
